...
public class KnifeParser {
    ...
    public static String toHtml(Spanned text,int knifeType) {
        StringBuilder out = new StringBuilder();
        switch (knifeType) {
            case KnifeText.FORMAT_BULLET:
                withinBulletThenQuote(out, text, 0, text.length());
                break;
            case KnifeText.FORMAT_QUOTE:
                withinQuoteThenBullet(out, text, 0, text.length());
                break;
            case KnifeText.CONTENT:
                withinContent(out, text, 0, text.length());
                break;
            case KnifeText.H1:
                withinH1(out,text,0,text.length());
                break;
            case KnifeText.H2:
                withinH2(out,text,0,text.length());
                break;
        }
//        withinHtml(out, text);
        return tidy(out.toString());
    ...
    private static void withinH1(StringBuilder out, Spanned text, int start, int end) {
        out.append("<h1>");
        withinContent(out, text, start, end);
        out.append("</h1>");
    }
    private static void withinH2(StringBuilder out, Spanned text, int start, int end) {
        out.append("<h2>");
        withinContent(out, text, start, end);
        out.append("</h2>");
    }
        ...
        for (int i = start; i < end; i = next) {
            ...
            for (int j = 0; j < spans.length; j++) {
                ...
                if (spans[j] instanceof AbsoluteSizeSpan){
                    AbsoluteSizeSpan sizeSpan= (AbsoluteSizeSpan) spans[j];
                    if (sizeSpan.getSize()==80) {
                        out.append("<h1>");
                    }else if (sizeSpan.getSize()==60){
                        out.append("<h2>");
                    }
                }
