...
public class KnifeText extends DeletableEditText implements TextWatcher {
    ...
    public static final int H1=0X08;
    public static final int H2=0X09;
    public static final int CONTENT=0X10;
    ...
    private int knifeTextType;

    public int getKnifeTextType() {
        return knifeTextType;
    }

    public void setKnifeTextType(int knifeTextType) {
        this.knifeTextType = knifeTextType;
    }
...
//    }
    ...
    protected void styleValid(int style) {
        switch (style) {
        ...
        getEditableText().setSpan(new StyleSpan(style), 0, length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
    }

    protected void styleInvalid(int style) {
        switch (style) {
            case Typeface.NORMAL:
            case Typeface.BOLD:
            case Typeface.ITALIC:
    ...
    protected boolean containStyle(int style) {
        int start=0;
        int end=length();
        switch (style) {
            case Typeface.NORMAL:
    ...
    //大标题
    public void setAbsoluteFontSpanH1() {
        int start=getSelectionStart();
        int end=getSelectionEnd();
        if (start >= end) {
            return;
        }
        getEditableText().setSpan(new AbsoluteSizeSpan(80),start,end,Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
    }

    //小标题
    public void setAbsoluteFontSpanH2() {
        int start=getSelectionStart();
        int end=getSelectionEnd();
        if (start >= end) {
            return;
        }
        getEditableText().setSpan(new AbsoluteSizeSpan(60),start,end,Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
    }

    protected void underlineInvalid() {
        int start=0;
        int end=length();
        if (start >= end) {
            return;
    ...
    protected void strikethroughValid() {
        int start=0;
        int end=length();
        if (start >= end) {
            return;
    ...
    protected void strikethroughInvalid() {
        int start=0;
        int end=length();
        if (start >= end) {
            return;
    ...
    public void link(String text,String link){
        URLSpan span = new KnifeURLSpan(link,linkColor,linkUnderline);
        SpannableString ss = new SpannableString(text);
        ss.setSpan(span, 0, text.length(), Spannable.SPAN_INCLUSIVE_INCLUSIVE);
        append(ss);
    }
    ...
    public boolean contains(int format) {
        switch (format) {
            case FORMAT_BOLD:
                return containStyle(Typeface.BOLD);
            case FORMAT_ITALIC:
                return containStyle(Typeface.ITALIC);
    ...
    public String toHtml() {
        ...
        return KnifeParser.toHtml(getEditableText(),knifeTextType);
    }
