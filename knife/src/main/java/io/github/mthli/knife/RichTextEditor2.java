...
public class RichTextEditor2 extends ScrollView {
	...
	private KnifeText lastFocusEdit; // 最近被聚焦的EditText
	...
	public RichTextEditor2(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		onEditorActionListener=new TextView.OnEditorActionListener() {
			...
			public boolean onEditorAction(TextView v, int actionId, KeyEvent event) {
				LinearLayout linearLayout= (LinearLayout) lastFocusEdit.getParent();
					int lastEditIndex = allLayout.indexOfChild(linearLayout);
		...
		View view = createEditText("input here",
				dip2px(EDIT_FIRST_PADDING_TOP));
		KnifeText firstEdit= (KnifeText) view.findViewById(R.id.knife);
	...
	private void onBackspacePress(EditText editTxt) {
		...
		if (startSelection == 0) {
			...
			if (null != preView) {
				...
				} else if (preView instanceof KnifeText) {
					// 光标EditText的上一个view对应的还是文本框EditText
					String str1 = editTxt.getText().toString();
					KnifeText preEdit = (KnifeText) preView;
	...
	public KnifeText getLastFocusEdit() {
		return lastFocusEdit;
	}

	public void setLastFocusEdit(KnifeText lastFocusEdit) {
	...
	private View createEditText(String hint, int paddingTop) {
		View view=inflater.inflate(R.layout.edit_item3,null);
		KnifeText editText  = (KnifeText) view.findViewById(R.id.knife);
...
//			lastFocusEdit.setSelection(editStr1.length(), editStr1.length());
	...
	private void addEditTextAtIndex(final int index, String editStr) {
		...
		KnifeText editText2= (KnifeText) view.findViewById(R.id.knife);
		editText2.setKnifeTextType(KnifeText.CONTENT);
	...
	public List<EditData> buildEditData() {
		...
		for (int index = 0; index < num; index++) {
			...
			if (itemView instanceof LinearLayout) {
				KnifeText item = (KnifeText) itemView.findViewById(R.id.knife);
				itemData.inputStr = item.toHtml();
	...
	public void strToEditData(String html){
//		html = Html.fromHtml(html).toString();
		int p=0;
		String[] s=html.split("<img");
		for (int i=0;i<s.length;i++) {
			String item=s[i];
			if (i!=0){
				item="<img"+item;
			}
			Log.e("strToEditData", item);

			List<String> imgs = getImgSrc(item);
			if (imgs.size() == 0) {

				item = Html.fromHtml(item).toString();
				addEditTextAtIndex(p,item);
				p++;
			} else {
				Log.e("strToEditData", imgs.toString());
				insertImage(null,imgs.get(0));
				item = delHTMLTag(Html.fromHtml("<img"+item).toString());
	...
	public void setupBullet(){
		KnifeText knifeText=getLastFocusEdit();
		LinearLayout linearLayout= (LinearLayout) knifeText.getParent();
		linearLayout.setBackgroundResource(R.drawable.bg);
		ImageView point= (ImageView) linearLayout.findViewById(R.id.img_point);
		point.setVisibility(GONE);
		knifeText.setTextSize(14);
		LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.WRAP_CONTENT, LinearLayout.LayoutParams.WRAP_CONTENT);
		lp.setMargins(15, 0, 0, 0);
		linearLayout.setLayoutParams(lp);
		knifeText.setKnifeTextType(KnifeText.FORMAT_BULLET);
//		knifeText.setTypeface(Typeface.DEFAULT);
		knifeText.setHint("无序引用");
	}
	...
	public void setupQuote(){
		...
		linearLayout.setLayoutParams(lp);
		knifeText.setKnifeTextType(KnifeText.FORMAT_QUOTE);
//		knifeText.setTypeface(Typeface.DEFAULT);
		knifeText.setHint("引用");
	}
...
//		knifeText.setTypeface(Typeface.DEFAULT);
		switch (type) {
			case KnifeText.H1:
				knifeText.setKnifeTextType(KnifeText.H1);
				knifeText.setTextSize(20);
				knifeText.setHint("大标题");
				break;
			case KnifeText.H2:
				knifeText.setKnifeTextType(KnifeText.H2);
				knifeText.setTextSize(16);
				knifeText.setHint("小标题");
				break;
			case KnifeText.CONTENT:
				knifeText.setKnifeTextType(KnifeText.CONTENT);
				knifeText.setTextSize(14);
				knifeText.setHint("正文");
				break;
		}
